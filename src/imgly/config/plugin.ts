...
/**
 * Version History Editor configuration plugin.
 *
 * Provides a design editing experience with version history functionality,
 * allowing users to save and restore design snapshots.
 *
 * The plugin overrides the default `saveScene` action to create version
 * history snapshots instead of downloading .scene files. Snapshots are
 * managed internally and can be accessed via `getSnapshots()` and
 * `subscribeToSnapshots()` from the imgly module.
 *
 * @public
 */
export class DesignEditorConfig implements EditorPlugin {
   ...
   */
  name = 'cesdk-version-history-editor';
  ...
  /**
   * Initialize the version history editor configuration.
   *
  ...
  async initialize({ cesdk, engine }: EditorPluginContext) {
    if (cesdk) {
      ...
      // Configure export, save (snapshot), and share actions
      // The saveScene action is overridden to create snapshots
